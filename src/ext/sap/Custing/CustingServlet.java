...
public class CustingServlet implements Controller {
		...
		for (int i = 1; i < finalList.size(); i++) {
			...
			if (!entity.isMaster()) {
				double sum = finalList.stream().filter(obj -> entity.getParent().equals(obj.getNumber()))
						.mapToDouble(obj -> (obj.getPrice() == null ? 0 : obj.getPrice())
								* (obj.getAmount() == null ? 1 : obj.getAmount()))
						.sum();
				if (sum > 0) {
					entity.setPrice(null);
				} else {
					entity.setPrice((entity.getPrice() == null ? 0 : entity.getPrice())
							* (entity.getAmount() == null ? 1 : entity.getAmount()));
				}
	...
	private static void loopAutoSapInfo(Tree tree, List<Map<String, String>> list, List<CustingEntity> returnList) {
		...
		Double cb = list.stream().filter(obj -> obj.get("MATNR").equals(number))
				.map(obj -> Double.parseDouble(obj.get("VERPR"))).collect(Collectors.toList()).get(0);
