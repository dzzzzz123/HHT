...
//				LOGGER.debug("attView Name:" + attView.getName());
	 ...
	 * @throws Exception
	 */
	public static String getObjType(Persistable obj) throws Exception {
		String type = TypeIdentifierUtilityHelper.service.getTypeIdentifier(obj).toString();
	 ...
	 * @return
	 */
	public static Enumeration<WTGroup> getAllGroupsFromOrg(String orgName) {
		OrgContainer newOrgContainer = null;
		try {
			QuerySpec queryspec = new QuerySpec(OrgContainer.class);
			queryspec.appendWhere(
					(WhereExpression) new SearchCondition(OrgContainer.class, "containerInfo.name", "=", orgName),
					new int[0]);
			QueryResult qr = PersistenceHelper.manager.find(queryspec);
			if (qr.hasMoreElements())
				newOrgContainer = (OrgContainer) qr.nextElement();
			PrincipalSpec localPrincipalSpec = new PrincipalSpec();
			try {
				localPrincipalSpec
						.setContainerReference(WTContainerRef.newWTContainerRef((WTContainer) newOrgContainer));
			...
			DirectoryContextProvider[] arrayOfDirectoryContextProvider = WTContainerHelper.service
					.getPublicContextProviders(localPrincipalSpec);
			Enumeration<WTGroup> allGroups = OrganizationServicesHelper.manager.findLikeGroups("*",
					arrayOfDirectoryContextProvider[0]);
			return allGroups;
