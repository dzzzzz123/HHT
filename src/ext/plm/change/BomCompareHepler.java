...
public class BomCompareHepler {
	...
	private static List<CompareMessage> getCompareMessage(WTPart parentPart1, WTPart parentPart2) {
		...
		String version1 = parentPart1.getVersionIdentifier().getValue() + "."
				+ parentPart1.getIterationIdentifier().getValue();
		String version2 = parentPart2.getVersionIdentifier().getValue() + "."
				+ parentPart2.getIterationIdentifier().getValue();
		// 0、更改物料
		...
		for (String sonPartNum : sonMap2.keySet()) {
			...
			if (part1 == null) {
				...
				String version = new_son_part.getVersionIdentifier().getValue() + "."
						+ new_son_part.getIterationIdentifier().getValue();
			...
			while (qr.hasMoreElements()) {
				...
				if (qr2.hasMoreElements()) {
					...
					if (hadPartAtt != null) {
						...
						Object[] obj = new Object[] { sonPart, usageLink.getQuantity().getAmount(),
								usageLink.getQuantity().getUnit().getDisplay(), usageLink };
	...
	private static CompareMessage compareSameSonPart(Object[] sonP1, Object[] sonP2) {
		try {
			...
			if (isUpdate) {
				...
				String version = part1.getVersionIdentifier().getValue() + "."
						+ part1.getIterationIdentifier().getValue();
				mess.setVersion(version);
				mess.setUnit_old(unit1);
				mess.setUnit_new(unit2);
				mess.setQuantity_old(quantiy1);
