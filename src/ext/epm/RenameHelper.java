 ...
 * 2D重命名逻辑：总是按3D的编号来重命名编号和文件名
 * Creo的3D和2D文件名和编号要改成小写，其他为大写
	...
	public static void renameEPMDocument(EPMDocument epm, String wtpartNumber,String newCADName) throws Exception {
		...
		try {
			...
			if(!(container instanceof PDMLinkProduct)){//只有产品库的才刷新文件名
				System.out.println("001-The EPMDocument is not in container PDMLinkProduct,but "+container.getContainerName()+",so return.");
				return ;
			}*/
			/*String version = epm.getVersionIdentifier().getValue();
			if(!StringUtils.equalsIgnoreCase(version, "A")){
			...
			/*if(!("CADCOMPONENT".equalsIgnoreCase(docType.toString()) || "CADASSEMBLY".equalsIgnoreCase(docType.toString())|| "CADDRAWING".equalsIgnoreCase(docType.toString()))){
				System.out.println("001-The Type of EPMDocument is not CADCOMPONENT or CADASSEMBLY or CADDRAWING ,so return.");
...
//				EPMDocumentHelper.service.changeCADName((EPMDocumentMaster) epm.getMaster(), newCADName);
				...
				PreparedStatement prepareStatement = connection.prepareStatement("update EPMDocumentMaster set CADName=?,documentnumber=? where ida2a2="+identified.getPersistInfo().getObjectIdentifier().getId());
	...
	private static void setEPMDocumentIBAType(WTPart part) throws Exception {
		...
		for(EPMDocument epm : epms) {
			String type = IBAUtil.getObjType(epm);
