...
public class SignaturePDF
{
  ...
  public static String processPDFSignByEPMDocumentDrawing(EPMDocument drawing, InputStream is, String pdfPath, String deleteKeyword)
    throws Exception
  {
    ...
    try
    {
      ...
      System.out.println("003-signature : getAssociatedProcesses from " + drawing.getName() + ":" + wfProcessFromPBO.size());
      if (wfProcessFromPBO.size() > 0) {
        WfProcess wfProcess = (WfProcess)wfProcessFromPBO.get(0);
        System.out.println("003-signature : the first  associatedProcesses is " + wfProcess.getTemplate().getName() + ">" + wfProcess.getName());
      ...
      e.printStackTrace();
      System.out.println("003-signature : signature " + (drawing != null ? drawing.getName() : "") + " error:" + e.getMessage());
      throw e;
    }
  }

  public static List<WfProcess> getWfProcessFromPBO(WTObject pbo)
    throws WTException
  {
    ...
    if ((pbo instanceof Changeable2))
    {
      ...
      if (changingChangeActivities.hasMoreElements())
      {
        ...
        if ((eca instanceof WTChangeActivity2)) {
          ...
          QueryResult qr = WfEngineHelper.service.getAssociatedProcesses(ca, 
            null, WTContainerRef.newWTContainerRef(WTContainerHelper.getContainer((WTContained)pbo)));
          if (qr.hasMoreElements()) {
            WfProcess wfprocess = (WfProcess)qr.nextElement();
            System.out.println("003-signature :getWfProcessFromPBO() ECA process found [" + ca.getNumber() + "-" + ca.getName() + "-" + wfprocess.getName() + "]");
    ...
    for (Iterator qr = promotionNotices.iterator(); qr.hasNext(); ) { Object object = qr.next();
        ...
        if ((object2 instanceof PromotionNotice)) {
          ...
          while (localQueryResult.hasMoreElements()) {
            ...
            if ((nextElement instanceof WfProcess)) {
              ...
              System.out.println("003-signature :getWfProcessFromPBO promotion notice process found [" + process.getName() + "]");
              break;
            }
          }
        }
    ...
    throws Exception
  {
    ...
    for (Iterator localIterator = keySet.iterator(); localIterator.hasNext(); ) { Object object = localIterator.next();
  ...
  public static void WriteSignatureToPDF(HashMap signInfoMap, InputStream is, String pdfPath, String deleteKeyword, String state,String objectType) throws Exception {
    try {
      if (is == null) {
...
//      PdfTextExtractor extractor = new PdfTextExtractor(reader);
      ...
      for (int j = 1; j < totalPage + 1; j++)
      {
        /*if (StringUtils.isNotBlank(deleteKeyword)) {
          System.out.println("003-signature : Search delete keyword [" + deleteKeyword + "] in page [" + j + "]");
          String textFromPage = extractor.getTextFromPage(j);
          if (StringUtils.contains(textFromPage, deleteKeyword)) {
            deletePageNumber = j;
            System.out.println("003-signature : delete page [" + deletePageNumber + "] found.");
          }
        }*/
        ...
        if ("N".equalsIgnoreCase(pdfSignPage[0])) {
          ...
          for (int k = 0; k < pdfSignPage.length; k++) {
            try {
              ...
              System.out.println("003-signature : signpage config from signature.properties has error :[" + pdfSignPage[k] + "] is not Integer number");
            }
          }
        }
      ...
      if (deletePageNumber == 1)
        pageList = pageList.append("1");
      else {
        for (int index = 1; index <= totalPage; index++) {
          if (index == deletePageNumber) {
            continue;
          }
      ...
      String pageString = pageList.toString();
      if (StringUtils.endsWith(pageString, ",")) {
        pageString = StringUtils.removeEnd(pageString, ",");
      }
      reader.selectPages(pageString);*/
      stamp.close();
      reader.close();
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static void WritePDFCoordinateContent(String objectType, String tufu, HashMap signInfoMap, PdfContentByte cb, String state)
    throws Exception
  {
    try
    {
      ...
      {
        System.out.println("003-signature : "+objectType+".SIGN.PROCEDURE config in signature.properties, no signature content.");
      }
  ...
  public static void contentWriteToPDF(Vector vec, String[] signInfo, String objectType, String signContentKey, PdfContentByte cb, StringTokenizer st, String signDateKey)
  {
    String userName = signInfo[0];
    String userLoginName = "";
    try
    {
      BaseFont bf = BaseFont.createFont(tempPath + PropertiesHelper.getStrFromProperties("PDF_FONT_PATH") + ",1", "Identity-H", true);
      ...
      SignatureHelper.writeStringContentToPDF(signInfo[3], cb, signContentKey+".Comment", bf, fontSize,objectType);
    }
    catch (Exception e)
    {
      e.printStackTrace();
